// Package semanticjson implements the relay's strict semantic-JSON and
// portability rules for internal consumers and the published document types.
package semanticjson
...
// DecodeJSON decodes one strict semantic-JSON value after expanding its exact
// decimal numbers into the form accepted by encoding/json's typed fields. It
// intentionally does not require the input bytes themselves to be canonical;
// callers that need that guarantee should use DecodeCanonicalJSON.
...
// its exact value. It is used by the event envelope's uint64 decoder.
func PlainSemanticNumber(value string) (string, error) { return plainSemanticNumber(value) }
