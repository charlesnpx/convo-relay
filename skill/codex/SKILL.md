---
name: relay
description: "Launch a relay dialogue between backend slots such as Codex CLI, Claude Code, Gemini CLI, or same-backend pairings using the convo-relay tool. Use when the user wants two independent agents to pressure-test, debate, or refine a topic — e.g. 'audit this', 'pressure-test the approach', 'help me think through this'."
---

You are initiating a structured dialogue between two backend slots using `convo-relay`. The usual pairing from this skill is Codex CLI plus Claude Code, but the tool can also run reversed ordering, Gemini pairings, same-backend pairings, or single-value shorthand like `--agents claude`, `--agents codex`, and `--agents gemini` when that is the right fit.
...
Before running anything, output exactly one approval-plan message and nothing else. That message must consist solely of a single `## Relay Plan` block following this template. Do not emit a separate preamble, analysis section, draft plan, or second plan.
...
If the user asks for changes before launch, replace the prior relay plan with one updated `## Relay Plan` message rather than appending another plan.

**Wait for the user to approve, modify, or redirect before proceeding.**

## Step 4: Run the relay

After the user approves, do not restate the plan and do not emit a second `## Relay Plan`. Run the command directly.
...
After the relay process finishes successfully, read the transcript file from the path printed by the command and present a structured summary:
...
- If the process is still running and the user did not ask for progress: keep waiting and tell the user the relay is still in progress.
...
- If the command is still running and the user did not ask for status: keep waiting; do not infer failure from stale session files alone
